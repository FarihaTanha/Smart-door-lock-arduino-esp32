...
//輸入WIFI連線帳號密碼
// Only Hotspot/AP mode will be used. No home WiFi connection.
String LineToken = "";

// Lock/relay output pin. GPIO12 requested.
// IMPORTANT: GPIO12 is a boot-strapping pin, so external relay circuit must NOT pull it HIGH during boot.
// Use a transistor/relay module with input kept LOW at startup and common GND.
byte pinLock = 12;

const unsigned long VERIFY_TIME = 500;      // wait 500ms after correct face
const unsigned long UNLOCK_TIME = 5000;     // unlock for 5 seconds
...
const char* appassword = "123456789";         //AP密碼至少要8個字元以上  
...
boolean controlState = true;   // true = face matched/no matched function active by default
...
digitalWrite(pinLock, LOW);  // শুরুতে lock
  ...
  // =============================
  // WiFi Hotspot/AP only
  // =============================
  WiFi.mode(WIFI_AP);
  delay(100);

  // Fixed hotspot IP: http://192.168.4.1
  ...
  if (apStarted) {
    ...
    Serial.println("Hotspot start failed. Restarting...");
    delay(1000);
    ESP.restart();
  }
...
void loop() {
  // If a correct face was seen, wait 500ms.
  // During this 500ms, if FaceNoMatched() appears, pendingUnlock becomes false.
  if (pendingUnlock && !isUnlocked && (millis() - verifyStartTime >= VERIFY_TIME)) {
    digitalWrite(pinLock, HIGH);
    unlockStartTime = millis();
    isUnlocked = true;
    pendingUnlock = false;

    Serial.print("UNLOCKED for 5 seconds. Face ID: ");
    Serial.println(pendingFaceId);
  }

  // After 5 seconds, lock again.
  if (isUnlocked && (millis() - unlockStartTime >= UNLOCK_TIME)) {
...
void FaceMatched(int faceid) {
  ...
  Serial.print("Correct face detected. Verifying 500ms. Face ID: ");
  Serial.println(faceid);

  // If already unlocked, do not restart the 5 second timer continuously.
  if (isUnlocked) return;

  // Start verification only once. Repeated matched frames will not reset the timer.
  if (!pendingUnlock) {
    pendingUnlock = true;
...
void FaceNoMatched() {
  ...
  Serial.println("Incorrect / No Match Found");

  // If incorrect face appears within 500ms verification time, cancel unlock.
  if (pendingUnlock && !isUnlocked) {
    pendingUnlock = false;
    pendingFaceId = -1;
    Serial.println("Unlock cancelled because incorrect face appeared within 500ms");
  }
}
...
<html>
    <head>
        ...
        <title>ESP32-CAM Door Lock AP</title>
        <style> 
...
static esp_err_t capture_handler(httpd_req_t *req){
    ...
    s = fmt2rgb888(fb->buf, fb->len, fb->format, out_buf);  //影像格式轉換RGB格式
    esp_camera_fb_return(fb);
    if(!s){
